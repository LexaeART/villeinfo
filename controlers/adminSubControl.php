...
if (isset($_POST['validInscrip'])) {
    if (isset($_POST['pseudo']) && !empty($_POST['pseudo'])) {
        ...
        if (isset($_POST['firstName']) && !empty($_POST['firstName'])) {
            ...
            if (!preg_match($regPseudo, $user->firstName)) {
                $formError['firstName'] = 'Retirer les caractères spéciaux';
            }
      }
  else {
      ...
      if (isset($_POST['lastName']) && !empty($_POST['lastName'])) {
          ...
          if (!preg_match($regPseudo, $user->lastName)) {
              $formError['lastName'] = 'Retirer les caractères spéciaux';
          }
    ...
    if (isset($_POST['validInscrip']) && count($formError) == 0) {
            ...
            if (isset($_POST['validInscrip']) && count($formError) == 0) {
                if (!$user->addUser()) {
                    ...
                    $insertSuccess = true;
                    ?>
                    <meta http-equiv="refresh" content="3;URL=adminLogin.php">
                    <?php
                }
            }
}
