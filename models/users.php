...
class users extends dataBase {
    ...
    public function loginIn() {
        $userLog = $this->db->prepare('SELECT `id`,`user`, `password` FROM '.self::PREFIX.'users WHERE `user` = :pseudo');
     ...
     */
     public function connectedInfos() {
         $connectedInfos = $this->db->prepare('SELECT `id`,`user`, `password`, `firstName`, `lastName` FROM '.self::PREFIX.'users WHERE `id` = :id');
     ...
     */
     ...
     public function allPseudoUser() {
         $query = 'SELECT `id`, `user` FROM '.self::PREFIX.'users';
     ...
     public function allOthersUsers() {
         $allOthersUsers = $this->db->prepare('SELECT `id`, `user`, `firstName`, `lastName` FROM '.self::PREFIX.'users WHERE `id` != :id');
    ...
    public function lastId() {
        $lastId = $this->db->prepare('SELECT MAX(`id`)+1 AS `lastId` FROM '.self::PREFIX.'users');
        $lastId->execute();
        if (is_object($lastId)) {
    ...
    public function usedPassword() {
        $usedPassword = $this->db->prepare('SELECT `password` FROM '.self::PREFIX.'users WHERE `id` = :id');
    ...
    public function addUser() {
        ...
        $query = 'INSERT INTO '.self::PREFIX.'users (`user`, `password`, `firstName`, `lastName`) VALUES(:pseudo, :password, :firstName, :lastName)';
    ...
    public function updateInfoUsers() {
        //On prépare la requête sql qui insert dans les champs selectionnés, les valeurs sont des marqueurs nominatifs
        $query = 'UPDATE '.self::PREFIX.'users SET user = :pseudo  WHERE id = :id';
