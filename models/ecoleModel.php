...
class ecoles extends dataBase {
    ...
    public function allEcoles() {
        $allEcoles = $this->db->query('SELECT `id`,`name`,`nameBoss`, `picture` FROM '.self::PREFIX.'ecoles');
        $allEcoles->execute();
        return $allEcoles = $allEcoles->fetchAll(PDO::FETCH_OBJ);
    }
    public function allProfs() {
        $query = 'SELECT `id`, `name` FROM '.self::PREFIX.'profs WHERE `idEcole` = :id';
    ...
    public function addSchool() {
        ...
        $query = 'INSERT INTO '.self::PREFIX.'ecoles(`name`, `nameBoss`, `picture`) VALUES(:name, :description, :picture)';
    ...
    public function addTeacher() {
        //On prépare la requête sql qui insert dans les champs selectionnés, les valeurs sont des marqueurs nominatifs
        $query = 'INSERT INTO '.self::PREFIX.'profs(`name`, `idEcole`) VALUES(:name, :id)';
        $addAssoc = $this->db->prepare($query);
        $addAssoc->bindValue(':name', $this->name, PDO::PARAM_STR);
    ...
    public function deleteSchool() {
        $query = 'DELETE FROM '.self::PREFIX.'ecoles WHERE `id` = :id';
    ...
    public function deleteTeacher() {
        $query = 'DELETE FROM '.self::PREFIX.'profs WHERE `idEcole` = :id';
        $savedHuntCount = $this->db->prepare($query);
        $savedHuntCount->bindValue(':id', $this->id, PDO::PARAM_INT);
        $savedHuntCount->execute();
        return $savedHuntCount;
    ...
    public function updateSchool() {
        //On prépare la requête sql qui insert dans les champs selectionnés, les valeurs sont des marqueurs nominatifs
        $query = 'UPDATE '.self::PREFIX.'ecoles SET name = :name,  nameBoss = :nameBoss, picture = :picture WHERE id = :id';
    ...
    public function updateTeacher() {
        //On prépare la requête sql qui insert dans les champs selectionnés, les valeurs sont des marqueurs nominatifs
        $query = 'UPDATE '.self::PREFIX.'profs SET name = :name WHERE id = :id';
        $udateAssoc = $this->db->prepare($query);
        $udateAssoc->bindValue(':id', $this->id, PDO::PARAM_STR);
        $udateAssoc->bindValue(':name', $this->name, PDO::PARAM_STR);
        //Si l'insertion s'est correctement déroulée on retourne vrai
